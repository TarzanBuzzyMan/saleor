...
from .forms import OnAccountPaymentForm

TEMPLATE_PATH = 'order/payment/onaccount.html'
...
def create_form(data, payment_information, connection_params):
    return OnAccountPaymentForm(data=data)


def authorize(payment_information: Dict, connection_params) -> Dict:
    success = dummy_ok_to_log_onaccount()
    error = None
    if not success:
